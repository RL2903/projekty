...
app.get("/zaloguj/:login/:password", (req, res) => {
    ...
    // poprawiłem brak `` ` `` wokół SQL i string w SQL musi być w apostrofach
...
// dzieki temu dodaje uzytkownika do bazy danych
app.get("/dodaj_uzytkownika/:login/:password/:rola", (req, res)=> {
    const {login, password, rola} = req.params;
    // poprawiłem brak `` ` `` i dodałem md5(password) jak funkcja
...
// dzieki temu dodaje sie sale do bazy
...
// dodaje rezerwacje do bazy danych
...
// pobiera rezerwacje z bazy danych na aplikacje
app.get("/rezerwacje", (req, res)=> {
    // poprawiłem brak `` ` `` i poprawiłem alias uzytkownicy (było uzytkownik)
    const sql = `SELECT rezerwacje.id, rezerwacje.data_godzina, uzytkownicy.login, sale.nazwa 
                 FROM rezerwacje 
                 JOIN uzytkownicy ON rezerwacje.uzytkownik_id = uzytkownicy.id 
                 JOIN sale ON rezerwacje.sala_id = sale.id`;
    con.query(sql, (err, result) => {
        if(err) res.json([]);
        else res.json(result);
    });
